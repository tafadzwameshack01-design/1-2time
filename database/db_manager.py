...
class _SQLiteBackend(_StorageBackend):
    ...
    def track_api_call(self, source_name: str, success: bool, error: str = "") -> None:
        ...
        existing = self._execute_read(
            "SELECT id, call_count, error_count, last_error FROM api_usage WHERE source_name=? AND usage_date=?",
            (source_name, today))
        if existing:
            row = existing[0]
            new_calls = (row.get("call_count") or 0) + 1
            new_errors = (row.get("error_count") or 0) + (1 if not success else 0)
            new_error_msg = error if not success else (row.get("last_error") or "")
            self._execute_write(
                "UPDATE api_usage SET call_count=?,error_count=?,last_call_at=?,last_error=? WHERE id=?",
                (new_calls, new_errors, now_iso, new_error_msg, row["id"]))
        else:
            self._execute_write(
                "INSERT INTO api_usage (source_name,usage_date,call_count,error_count,last_call_at,last_error) VALUES (?,?,1,?,?,?)",
                (source_name, today, 1 if not success else 0, now_iso, error if not success else ""))
    ...
    def save_team_ratings(self, ratings: list) -> None:
        ...
        for r in ratings:
            ...
            existing = self._execute_read(
                "SELECT id FROM team_ratings WHERE team_id=? AND league_key=?",
                (team_id, league_key))
            if existing:
                self._execute_write(
                    "UPDATE team_ratings SET attack_param=?,defense_param=?,home_advantage=?,last_updated=? WHERE team_id=? AND league_key=?",
                    (r.get("attack_param"), r.get("defense_param"), r.get("home_advantage"), now_iso, team_id, league_key))
            else:
                self._execute_write(
                    "INSERT INTO team_ratings (team_id,team_name,league_key,attack_param,defense_param,home_advantage,last_updated) VALUES (?,?,?,?,?,?,?)",
                    (team_id, r.get("team_name",""), league_key, r.get("attack_param"), r.get("defense_param"), r.get("home_advantage"), now_iso))
